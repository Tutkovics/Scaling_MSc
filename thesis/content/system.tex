...
%TODO
Korábban készítettem egy Docker képfájlt, ami a jelenlegi rendszerben felhasználttal azonos céllal született, hogy szabadon konfigurálható paraméterezés alapján szolgálja ki a beérkező kéréseket. Az így megírt Go alapú webszervert is tovább kellett fejleszteni, személyre szabni, hogy valósághűbb eredményeket szolgáltasson.
...
A teljes kódbázis és annak működésének részletekbe menő bemutatását nem érzem releváns részének a végső rendszer felépítését tekintve. 
...
A forgalom generálását egy külön alkalmazás végzi, a \verb+Vegeta+\citep{vegetaGithub}. 
Természetesen nagyon sok forgalom generáló alkalmazást tudunk felhasználni és a kész rendszer korábbi verziójában mást használtunk.
A korábbi döntés a \verb+Fortio+ generátorra esett, mert egyre szélesebb körben használják és ideális döntésnek tűnt a méréseinkhez.
...
Ebben nagy segítségemre volt egy összegyűjtött alkalmazás lista\citep{benchmarkToolsList} így megkeresni már nem csak össze kellett hasonlítani őket.
...
Például meg kell adni az aktuális QPS értéket, amit szeretnénk elérni, hogy mennyi ideig fusson a mérés, hogy hova és milyen néven mentse a kapott eredményeket, és a legfontosabb, hogy hova küldje a kéréseket.
